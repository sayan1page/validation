...
def pytest_generate_tests(metafunc):
    ...
    rows = mysql_instance.get_data("select pub_id,targeting_info from pub_bid_probability_info_1 limit 10")
    argnames = ['key','value']
    argvalues = []
    print("hello") 
    for row in rows:
        print("hi")
        data = json.loads(row["targeting_info"])
        pid = row["pub_id"]
