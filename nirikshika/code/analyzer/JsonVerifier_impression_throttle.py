...
def pytest_generate_tests(metafunc):
    f = open("/home/ml.user/QA/Sayan/nirikshmika/config/impressionthrottle.config_json")
    config = {}
    for line in f:
    ...
    rows = mysql_instance.get_data("select pub_id,targeting_info from KomliAdServer.pub_impression_throttling_info where ml_model = 1 and targeting_info IS NOT NULL;")
    argnames = ['key','value']
    argvalues = []
    for row in rows:
