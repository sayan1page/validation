...
class MSSqlDbWrapper(object):
	...
	def __init__(self, configpath):
		...
		self.engine = sql.create_engine ("mssql+pyodbc://{0}:{1}s@subschurnpoc.database.windows.net/SubsChurn?driver=SQL+Server+Native+Client+11.0".format(quote_plus(config['mssql_usrname']), quote_plus(config['mssql_passwd'])))
